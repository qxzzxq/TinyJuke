...
- **About screen QR codes (`RELEASE_URL` / `PROJECT_URL` + `drawQrCode()`):** the About screen (menu item 7, formerly "Version") is **paged** — two pages with an iOS-style dot indicator at `y=289`, rotation flips between them, click or hold returns to the menu. Page 1 is the old Version screen: firmware version, build mode, and a QR pointing at `https://github.com/qxzzxq/TinyJuke/releases/latest` — the `/latest` alias rather than a pinned tag, so the code stays correct as versions ship and the device never needs to be online to know what the newest release is. Page 2 is the project page: a `made with ♥ by qxzzxq` credit and a QR to `PROJECT_URL` (the repo root). Encoded with `ricmoo/QRCode` (pure C, no display coupling, so it composes with Arduino_GFX; the display-bound QR libraries on the registry do not). Three things here are load-bearing:
  ...
  Placement lives in the pure `qr_layout.h` so the quiet zone and integer module scaling are host-tested; a fractional scale would make some modules a pixel wider than others and smear the edges scanners measure. **The two pages' QR bands are sized so `qrPlace()` lands on scale 4 (164 px) on both** — page 1 gets `y=96 h=172`, page 2 `y=104 h=164`. Adding the dot row cost the vertical slack that used to absorb a sloppy band: one pixel short and that page silently drops to scale 3, shrinking its symbol by a quarter while the other stays large. `test_about_page_qr_bands_both_land_on_scale_four` pins both bands and their clearance from the caption line.

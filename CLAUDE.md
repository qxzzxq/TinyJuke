...
- `!found && tagPresent` → tag removed: `TAG_ABSENT_CONFIRM`-miss debounce, then clear `tagPresent`/`sleepStopped`, stop playback, draw waiting.
...
- NFC poll cadence: the awake waiting-screen poll blocks for `NFC_POLL_MS` (100 ms) per iteration, which sets the loop cadence. `TAG_ABSENT_CONFIRM` (9, in `jukebox_state.h`) is sized against it to keep removal tolerance near 900 ms — **change the two together**. While a button press is being tracked on the idle waiting screen (`encPressActive() && !tagPresent`) the poll is skipped entirely, so the loop free-runs and the hold indicator animates smoothly; that's safe precisely because no tag is present, so the removal debounce isn't armed and a skipped poll can't read as a tag disappearing.

...
static void handleRoot() {
  ...
  html += "<h2>Tag Mappings</h2>"
          "<table><tr><th>UID</th><th>File</th><th></th></tr>";
  ...
  if (tagDoc.size() > 0) {
    ...
    html += "<tr><td colspan=3>No tags mapped</td></tr>";
  }
  html += "</table>";
  ...
  html += "<h3>Link New Tag</h3>"
          ...
          "pattern='[0-9A-Fa-f:]+' required style='width:160px'> "
  ...
  if (dir && dir.isDirectory()) {
    ...
    while ((f = dir.openNextFile())) {
      if (!f.isDirectory()) {
        ...
        const char *base = strrchr(name, '/');
        if (base) base++; else base = name;
        // Value stored as "music/filename" (relative path for tags.json)

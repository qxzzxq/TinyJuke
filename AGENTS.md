...
├── qr_layout.h       — Pure QR placement: quiet-zone-aware integer scaling + centring for the About screen's QR codes (testable on native)
├── jukebox_state.h/.cpp — Pure top-level FSM (Waiting / Sleeping + sleepStopped / tagPresent flags) driving loop()'s decisions
├── gui.h/.cpp        — Management mode (menu + volume/brightness/theme/powersave/sleeptimer/about/web/bluetooth screens)
...
- Management mode active (`guiActive()`) → delegate to `guiLoop()` (menu, volume, brightness, power saving, sleep timer, about, web server, bluetooth, reboot). Menu items: **Web Management, Bluetooth Mode, Volume, Brightness, Color Theme, Power Saving, Sleep Timer, About, Reboot** (9 items; menu `itemH` is 28 px to fit them all above the hint bar). Reboot shows a confirmation screen — hold = `ESP.restart()`, click/rotate = cancel back to menu.
...
- **About screen QR codes (`RELEASE_URL` / `PROJECT_URL` + `drawQrCode()`):** the About screen (menu item 7, formerly "Version") is **paged** — two pages with an iOS-style dot indicator at `y=289`, rotation flips between them, click or hold returns to the menu. Page 1 is the old Version screen: firmware version, build mode, and a QR pointing at `https://github.com/qxzzxq/TinyJuke/releases/latest` — the `/latest` alias rather than a pinned tag, so the code stays correct as versions ship and the device never needs to be online to know what the newest release is. Page 2 is the project page: a `made with ♥ by qxzzxq` credit, the same QR band pointing at `PROJECT_URL` (the repo root), and the bare repo URL below it in `C_TEXT` rather than `C_MUTED` — that line is meant to be read and typed, not to caption the QR. Encoded with `ricmoo/QRCode` (pure C, no display coupling, so it composes with Arduino_GFX; the display-bound QR libraries on the registry do not). Three things here are load-bearing:
  ...
  3. **`qrcode_initBytes()` never range-checks the payload.** Over-long input overruns the buffer rather than returning an error (it segfaults on the host), so the `static_assert`s on `RELEASE_URL` **and** `PROJECT_URL` are the *only* guard against memory corruption — not a nicety. Never weaken them, and never rely on `qrcode_initText()`'s return value to catch an oversized string. Any new QR payload needs its own assert.
  Placement lives in the pure `qr_layout.h` so the quiet zone and integer module scaling are host-tested; a fractional scale would make some modules a pixel wider than others and smear the edges scanners measure. **Both pages hand `drawQrCode()` the same band** (`ABOUT_QR_BAND_Y=96` / `_H=172`), so the two symbols are pixel-identical and flipping pages moves only the text around them. The band is sized so `qrPlace()` lands on scale 4 (164 px); adding the dot row cost the vertical slack that used to absorb a sloppy band, and one pixel short silently drops to scale 3, shrinking the symbol by a quarter. **Each page gets at most two text blocks: one above the QR, one caption below.** A revision that also captioned page 2 *above* its QR left 3 px of background between the two, and the symbol read as cramped and smaller than page 1's despite being the same 164 px — which is why `test_about_qr_band_lands_on_scale_four_with_room_to_breathe` asserts an 8 px minimum clearance rather than mere non-overlap.
  The heart in the credit line is **`0x03` from the classic CP437 glyph table** that Arduino_GFX ships as its default font, drawn in `C_RED` by printing the line in two pieces. `write()` passes every byte except `\n`/`\r` straight to `drawChar()`, so it needs no UTF-8 handling — this does not contradict the ASCII filter on AVRCP metadata, which exists to reject *multi-byte sequences*, not the CP437 range. `charBounds()` counts it as a normal 6×8 cell, so `textWidth()` centres it correctly.
